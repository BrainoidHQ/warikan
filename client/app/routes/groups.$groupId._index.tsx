  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4">
        <Input isReadOnly label="タイトル" defaultValue={group.title} />
          ...
          <CardBody>
            <Listbox>
              {group.warikan
                .filter(
                  (warikan) =>
                    warikan.from.id === user.id || warikan.to.id === user.id,
                )
                .map((warikan) =>
                  warikan.from.id === user.id
                    ? `${warikan.to.name}に${warikan.amount}円おくる`
                    : `${warikan.from.name}から${warikan.amount}円もらう`,
                )
                .map((message) => (
                  <ListboxItem key={message}>{message}</ListboxItem>
                ))}
            </Listbox>
          </CardBody>
        </Card>
